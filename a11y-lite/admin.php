...
class Accessibility_Lite_Admin {
	...
	public static function sanitize( $in ) {
		...
		$out['vpos_desktop']    = in_array( $in['vpos_desktop'], array( 'top', 'middle', 'bottom' ), true ) ? $in['vpos_desktop'] : 'middle';
		$out['vpos_desktop_vh'] = isset( $in['vpos_desktop_vh'] ) ? absint( $in['vpos_desktop_vh'] ) : 50;
		$out['vpos_desktop_vh'] = min( 100, $out['vpos_desktop_vh'] );
		$out['vpos_mobile']     = in_array( $in['vpos_mobile'], array( 'top', 'middle', 'bottom' ), true ) ? $in['vpos_mobile'] : 'bottom';
		$out['vpos_mobile_vh']  = isset( $in['vpos_mobile_vh'] ) ? absint( $in['vpos_mobile_vh'] ) : 15;
		$out['vpos_mobile_vh']  = min( 100, $out['vpos_mobile_vh'] );
		...
		$out['save_exp']  = isset( $in['save_exp'] ) ? absint( $in['save_exp'] ) : 720;
			...
			<form method="post" action="options.php">
				...
				<table class="form-table" role="presentation">
					...
					<tr>
						...
						<td>
							<label><input type="checkbox" name="<?php echo esc_attr( self::OPTION ); ?>[skip_link]" value="1" <?php checked( $o['skip_link'], 1 ); ?> /> <?php esc_html_e( 'Show a "Skip to content" link (accesskey: s)', 'a11y-lite' ); ?></label>
						</td>
					</tr>
					<tr>
